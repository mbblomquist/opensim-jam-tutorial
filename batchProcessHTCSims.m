%% Batch Process High throughput Simulation(s)
%==========================================================================
...
function StoData = batchProcessHTCSims( Params )

% Results Directory
resultsDir = Params.resultsDir ;
resultsTarFile = Params.resultsTarFile ;
...
for iMdl = 1 : Params.numModels
    ...
    for iTrial = 1 : length( trialNames )
        ...
        tempSimFld = fullfile( resultsDir, trialNames{ iTrial } , 'results', num2str( iMdl - 1 ) ) ; % temp simulation file name

        % untar results and delete tar file if exists
        if exist( fullfile( tempSimFld , resultsTarFile), 'file' )
            untar( fullfile( tempSimFld, resultsTarFile ), tempSimFld )
            delete( fullfile( tempSimFld, resultsTarFile ) )
        end
